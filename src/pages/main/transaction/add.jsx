...
function TransactionAddPage() {
    ...
    const attachmentOptions = attachments.map((a) => ({ value: a.id, label: a.filename }))
    const budgetOptions = budgets.map((b) => ({ value: b.id, label: b.name }))

    const addPending = (setter, source) => (id) => {
        const item = source.find((entry) => entry.id === id)
        if (item) setter((prev) => [...prev, item])
    }
    const removePending = (setter) => (item) => setter((prev) => prev.filter((entry) => entry.id !== item.id))
        ...
        try {
            ...
            for (const tag of pendingTags) {
                try {
                    ...
                    linkFailures += 1
                    toast.error(`Couldn't link tag "${tag.name}": ${error.message}`)
                }
            }
            for (const attachment of pendingAttachments) {
                try {
                    ...
                    linkFailures += 1
                    toast.error(`Couldn't link attachment "${attachment.filename}": ${error.message}`)
                }
            }
            for (const budget of pendingBudgets) {
                try {
                    ...
                    linkFailures += 1
                    toast.error(`Couldn't link budget "${budget.name}": ${error.message}`)
                }
            }
