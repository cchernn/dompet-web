...
const parseFilterDate = (value) => (value ? parse(value, "yyyy-MM-dd", new Date()) : undefined)
...
function SortableHeader({ column, label, sort, onToggle, className }) {
...
function TransactionListPage() {
    ...
    const accountOptions = [{ value: ALL, label: "Any" }, ...accounts.map((a) => ({ value: a.name, label: a.name }))]
    const tagOptions = [{ value: ALL, label: "Any" }, ...tags.map((t) => ({ value: t.name, label: t.name }))]
    const budgetOptions = [{ value: ALL, label: "Any" }, ...budgets.map((b) => ({ value: b.name, label: b.name }))]
    ...
    const handleDeactivate = async (transaction) => {
        try {
                                                                ...
                                                                <Popover>
                                                                    <PopoverTrigger asChild>
                                                                        <button type="button" className="inline-flex items-center gap-1 hover:text-foreground">
                                                                            <Tag className="size-3" />{tx.tags.length}
                                                                        </button>
                                                                    </PopoverTrigger>
